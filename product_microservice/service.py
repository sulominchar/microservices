...
def authenticate_user():
    ...
    for user in users:
        if user['username'] == username and user['password'] == password:
            ...
            response.set_cookie('token', token) #send the token in the cookies
            return response, 200
    return jsonify({'error': 'Invalid username or password'}), 401


@app.route('/products', methods=['GET'])
@token_required #calling custom decorator to handling authentication
def get_products(current_user_id):
    headers = {'Authorization': f'Bearer {request.cookies.get("token")}'} #taking the auth token from the cookies
    response = requests.get(f"{BASE_URL}/products", headers=headers)
    if response.status_code != 200:
        return jsonify({'error': response.json()['bad request']}), response.status_code
    products = []
